...
La facilitación entre especies promueve el establecimiento exitoso de especies raras al proporcionar beneficios a través de interacciones positivas entre individuos, lo que a su vez reduce las disparidades de rendimiento entre los adultos de diferentes especies.
...
```{r crd_anfibios}
sitios = c("Puyo", "Tena", "Santa Clara")

```



# Crear un vector con las localidades del Ecuador
sitios <- c("Puyo", "Tena", "Santa Clara")

# Crear un marco de datos vacío para almacenar los datos
datos <- data.frame(Localidad = character(),
                    Especie = character(),
                    Variable1 = numeric(),
                    Variable2 = numeric())
...
for (sitio in sitios) {
  # Generar los datos correspondientes a cada localidad
  # Aquí deberías agregar el código necesario para generar los datos
  # basándote en el diseño del artículo
  
  # Agregar los datos generados al marco de datos principal
  datos_localidad <- data.frame(Localidad = sitio,
                                Especie = # agregar especies generadas,
                                Variable1 = # agregar valores de Variable1 generados,
                                Variable2 = # agregar valores de Variable2 generados)
  datos <- bind_rows(datos, datos_localidad)
}
